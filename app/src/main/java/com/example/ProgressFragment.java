...
public class ProgressFragment extends Fragment {
    ...
    private void setupProgressPhotosRecycler() {
        photoAdapter = new ProgressPhotoAdapter(requireContext(), photo -> {
            new AlertDialog.Builder(requireContext())
                    .setTitle("Delete Progress Photo")
                    .setMessage("Remove this photo entry?")
                    .setPositiveButton("Delete", (dialog, which) -> {
    ...
    private void showAddWeightDialog() {
        ...
        dialogBinding.btnSubmitWeight.setOnClickListener(v -> {
            ...
            if (insertedId > 0) {
                Toast.makeText(requireContext(), "Weight saved! SQLite Trigger updated user profile.", Toast.LENGTH_SHORT).show();
                dialog.dismiss();
                loadAllProgressData();
            } else {
